...
BOOST_AUTO_TEST_CASE(testStructWithOptionalNumber) {
  typedef MemberSequenceRule<StructWithOptional,
    MemberRule<StructWithOptional,
               int,
               &StructWithOptional::n1,
               NumberRule>,
    MemberRule<StructWithOptional,
               Optional<int>,
               &StructWithOptional::n2,
               OptionalRule<int, NumberRule>
  > > StructWithOptionalRule;

  StructWithOptional s;
  size_t read;

  BOOST_CHECK_EQUAL(StructWithOptionalRule::fromString(s, "19 86", read),
                    ResultOk);
  ...
  BOOST_CHECK_EQUAL(StructWithOptionalRule::fromString(s, "19 none", read),
                    ResultOk);
  BOOST_CHECK_EQUAL(s.n1, 19);
...
BOOST_AUTO_TEST_CASE(testArrayRule) {
  ...
  BOOST_CHECK_EQUAL(NumberArrayRule::fromString(array, "1, 2, 3", read),
                    ResultOk);
  BOOST_CHECK_EQUAL(read, 7);
...
BOOST_AUTO_TEST_CASE(testStructWithArrayParser) {
  typedef MemberSequenceRule<StructWithArray,
    ...
    MemberRule<StructWithArray,
               std::vector<int>,
               &StructWithArray::an,
               ArrayRule<int, NumberRule>
  > > StructWithArrayRule;

  size_t read;
  StructWithArray s;

  BOOST_CHECK_EQUAL(StructWithArrayRule::fromString(s, "1 2, 3, 4", read),
                    ResultOk);
...
BOOST_AUTO_TEST_CASE(testAudioCodecs) {
  typedef MemberSequenceRule<AudioCodecs::Codec,
    ...
    MemberRule<AudioCodecs::Codec,
               unsigned int,
               &AudioCodecs::Codec::mask,
               HexRule>,
    MemberRule<AudioCodecs::Codec,
               unsigned int,
               &AudioCodecs::Codec::latency,
               HexRule>
  ...
  BOOST_CHECK_EQUAL(
      AudioCodecsRule::fromString(codecs, "LPCM 00000003 01", read),
      ResultOk);
