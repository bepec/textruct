...
struct NumberRule {
  static Result fromString(int& destination,
...
struct HexRule {
  static Result fromString(unsigned& destination,
                           const std::string& text,
                           size_t& read) {
    const char *start = text.c_str();
    char *end;
...
struct OptionalRule {
  static Result fromString(Optional<ContentType>& destination,
                           const std::string& text,
                           size_t& read) {
    ...
    if (text.size() >= 4 && text.substr(0, 4) == "none") {
      ...
      result = ContentRule::fromString(destination.content, text, read);
    }
...
struct ArrayRule {
  static Result fromString(std::vector<Type>& destination,
                           const std::string& text,
                           size_t& read) {
    bool match = true;
    read = 0;
...
struct EnumRule {
  static Result fromString(Enum& destination,
                           const std::string& text,
                           size_t& read) {
    Result result = ResultError;
